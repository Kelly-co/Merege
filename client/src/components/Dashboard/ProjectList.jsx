  ...
  function allkeyfunction() {
    projects.map((project) => {
      return Object.assign(project, {
        ...
        languages: [project.language1, project.language2, project.language3],
      });
    });
    console.log(projects);
  }
    ...
    if (e.target.value === "All beaches") {
      ...
      const searchProject = projects.filter((project) => {
        ...
        for (let i = 0; i < project.languages.length; i++) {
          if (
            project.languages[i].toLowerCase().startsWith(query.toLowerCase())
          ) {
            return true;
          }
        }
        for (let i = 0; i < project.keyword.length; i++) {
          if (
            project.keyword[i].toLowerCase().startsWith(query.toLowerCase())
          ) {
            return true;
          }
        }
    ...
    <div className="dashboard">
      ...
      <div>
        <select className="select-css" onChange={handleFilterBranch}>
          ...
          {[
            ...new Map(projects.map((item) => [item.branch, item])).values(),
          ].map((element) => (
            <option>{element.branch}</option>
          ))}
        </select>
      </div>
      <div>
      ...
      <div>
        <button className="button" onClick={handleNewProject}>
          +
        </button>
      </div>
    </div>
